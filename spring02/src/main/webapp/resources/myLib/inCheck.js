...
function pwCheck(){
	...
	if(pw.length <4 || pw.length > 10){
		...
		return false;
	} else if(pw.replace(/[a-z.0-9.\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi,'').length > 0){
		document.getElementById('pMessage').innerHTML='password는 영문,숫자,특수문자만 가능합니다';
		return false;
	} else if(pw.replace(/^[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi,'').length > 1){
		document.getElementById('pMessage').innerHTML='특수문자를 1개 이상 포함시켜주세요';
		return false;
	} else{
...
// 7 bitrhday
function bdCheck(){
	let bitrhday = document.getElementById('birthday').value;
