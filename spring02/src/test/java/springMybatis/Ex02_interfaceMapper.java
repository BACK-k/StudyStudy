...
public class Ex02_interfaceMapper {
	// ** interface Mapper 설정
	// => Controller -> Service -> (DAO) -> interface Mapper
	// : xml의 sql 구문을 이용해서 DB처리
	@Autowired
	MemberMapper mapper;
	...
	// 이를 위한 경로 설정 : <mybatis-spring:scan base-package="mapperInterface"/>
	@Autowired
	MemberDAO dao;
	@Autowired
	MemberDTO dto;
...
//	@Test
	public void selectDTO() {
		...
		dto2.setId("banana");
		// Mybatis는 참조형 매개변수 사용시 매개변수 주소를 공유하지않아 dto가 비어있음
//		mapper.selectDTO(dto2);
		// 컨트롤러에서 사용한 dto를 담기 위해, 리턴한 값을 참조형변수 dto에 넣어줌
		dto2 = mapper.selectDTO(dto2);
		System.out.println(" Mybatis selectDTO() => " + dto2);
	}

	// 3 복수의 매개변수 사용 Test
	// Mybatis에서 2개이상의 매개변수 처리
