...
function rsLoginf(){
	...
	`<table align center>
		...
		<tr height=40>
			<td gdcolor="aqua"><label for="id">ID</label></td>
			<td><input type="text" name="id" id="id"></td>
		</tr>
		<tr height=40>
			<td gdcolor="aqua"><label for="password">password</label></td>
...
function rsLogin(){
	...
	fetch(url, {method : 'post',
				// stringify : JavaScript 값을 JSON 문자열로 변경 
				body : JSON.stringify({
	...
	// 서버에서 결과값을 Text로 보내 text() 사용, then으로 return
	// Type 별로 Body-reading method를 적용
	...
	// => 1단계에서 return 한 Data 처리
		...
		if(err.message == '502') alert('id 또는 passwor 오류입니다.');
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// 2. Axios Login
// => 라이브러리 추가 (CDN 으로..   axTestForm01.jsp 에)
...
function axiLoginjj(){
	...
	axios({
		...
		else alert(` 시스템 오류입니다. status => ${err.message} `);
	});
} // axiLoginjj
// ***************************************************************
// *** JSON 객체
