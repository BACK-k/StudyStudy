...
public class MemberDSLRepositoryImpl implements MemberDSLRepository {
	...
	// 1 Entity return
	// Q클래스로 SQL구문 작성하고 Entity return
	// Parameter로 전달된 조원들만 출력하기
	@Override
	public List<Member> findMemberJnoDSL(int jno) {
		// Q에 있는 member
		return jPAQueryFactory.selectFrom(member).where(member.jno.eq(jno).and(member.point.goe(4000)))
	...
	public List<MemberDTO> findMemberJoinDSL() {
		return jPAQueryFactory
				.select(Projections.bean(MemberDTO.class, member.id, member.name, member.jno, jo.jname, jo.project))
				.from(member).leftJoin(jo).on(member.jno.eq(jo.jno)).fetchJoin().fetch();
	}
	...
	public List<MemberDTO> findMemberJoinDSL2() {
		return jPAQueryFactory
				.select(Projections.fields(MemberDTO.class, member.id, member.name, member.jno, jo.jname, jo.project))
				.from(member).leftJoin(jo).on(member.jno.eq(jo.jno)).fetchJoin().fetch();
	}
}
