...
@Service
@Log4j2
// 각각의 필드에 대해서 하나의 매개변수가 있는 생성자를 만들어줌
//=> 초기화 되지 않은 모든 final 필드에만 적용됨. 
@RequiredArgsConstructor
public class GuestbookServiceImpl implements GuestbookService {

	// JPA SQL 처리를 위해 정의한 interface
	...
	public PageResultDTO<GuestbookDTO, Guestbook> pageList(PageRequestDTO requestDTO) {
		...
		// Function<EN, DTO> 정의 -> 생성자에게 전달
		// 익명 클래스 : 후손 클래스가 해당 위치에서만 사용되는 경우
		// new FunctionClass() 처럼 미리 만들어놓은 구현 객체 대신
		// new 조상의 이름{ 클래스 구현 } 해서 사용
		// 조상이 하나밖에 없다면 조상의 이름까지 생략 가능
...
//		return new PageResultDTO<>(result, entity -> entityToDto(entity));
	...
	public Guestbook selectOne(Long gno) {
		...
		// => isPresent() : Optional객체에 저장된 값이 null인지 확인(false -> null)
